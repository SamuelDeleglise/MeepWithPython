...
class epsilon(PolygonCallback2D):
    def layer(self,ymin,ymax):
        pol_points = numpy.zeros((5,2))
        pol_points[0] = [0.0, ymin]
        pol_points[1] = [gridSizeX,ymin]
        pol_points[2] = [gridSizeX, ymax]
        pol_points[3] = [0.0, ymax]
        pol_points[4] = [0.0, ymin]
        return pol_points
    ...
    def __init__(self,normalize = False):
        ...
        layers_high = []
        layers_low = []
        thickness = 0.25 #in units of lambda
        for i in range(10):
            layers_high.append(self.layer(gridSizeY/2 + i*1.5*thickness,gridSizeY/2 + i*1.5*thickness + thickness/2))
            layers_low.append(self.layer(gridSizeY/2 + i*1.5*thickness,gridSizeY/2 + i*1.5*thickness + thickness))
            
            
        if not normalize:
            for i,j in zip(layers_high,layers_low):
                self.add_polygon(i, 2.0)
                self.add_polygon(j, 1.0)
...
#define a gaussian line source of length 'wgWidth' at X=wgLength/2, Y=padSize
    ...
    return the_fields,input_flux,output_flux
...
my_fields,input_flux,output_flux = make_fields()
my_fields_cal,input_flux_cal,output_flux_cal = make_fields(calibre = True)

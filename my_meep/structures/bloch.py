...
class epsilon(Callback):
    ...
    def double_vec(self, r):
        ...
        (cx, cy) = (gridSizeX/2,gridSizeY/2)
        radius = 0.36
        if  ((cx-r.x())**2 + (cy-r.y())**2)  < radius**2:
            return 1.
        else:
            return 13.
...
#use_averaging(False)  #--> uncomment this line to disable eps-averaging!
    ...
    my_structure = structure(my_space, EPS, pml(1.0,Y), symmetry = sym)
...
#    set_AMPL_Callback(af.__disown__())
    the_fields.add_point_source(Ez, srcGaussian, vec(gridSizeX/2.4321,gridSizeY/2.4321))

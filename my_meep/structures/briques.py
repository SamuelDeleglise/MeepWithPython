...
srcPulseWidth = 1.9 #gaussian source pulse width
...
class epsilon(Callback):
    ...
    def double_vec(self, r):
        ...
        height = 1.2
        if r.y() > gridSizeY/2 + height/2:
            return 1.
        if r.y() < gridSizeY/2 - height/2:
            return 1.

        width = 0.8
        if r.x() > gridSizeX/2 + width/2:
            return 1.
        if r.x() < gridSizeX/2 - width/2:
            return 1.
...
def make_fields(calibre = False,bloch_k = 0.0,f = srcFreqCenter,df = srcPulseWidth,symmetry = None):

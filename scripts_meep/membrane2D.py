...
class membrane2D(My_structure):
    def __init__(self,height = 1.2,width = 0.7,n = 3.0,centerX = 0.0,centerY = 0.0,centerZ = 0.0):
        My_structure.__init__(self,centerX,centerY,centerZ)
        self.height = height
        self.width = width
        self.n = n
    ...
    def double_vec(self, r):
        n = self.n**2
        if r.z() > self.centerZ + self.height/2.:
            return 1.
        if r.z() < self.centerZ - self.height/2.:
            return 1.
        ...
        if r.y() > self.centerY + self.width/2.:
            return n
        if r.y() < self.centerY - self.width/2.:
            return n
...
space = My_space(gridSizeX,gridSizeY, res, f, df,boundary_conditions = pml(1.0,Z),periodic_directions = [X,Y],bloch = vec(0,0,0),gridSizeZ = gridSizeZ)

membrane = membrane2D(height = 0.627, width = sqrt(1.0-0.62), n = 3.214, centerX = gridSizeX/2., centerY = gridSizeY/2.,centerZ = gridSizeZ/2.)
space.add_structure(membrane)

space.add_flat_source(vol = volume(vec(0,0,border),vec(gridSizeX,gridSizeY,border)),comp = Ex)

space.add_a_flux("input",volume(vec(0,0,2*border),vec(gridSizeX,gridSizeY,2*border)))
space.add_a_flux("output",volume(vec(0,0,gridSizeZ - 2*border),vec(gridSizeX,gridSizeY,gridSizeZ-2*border)))
##=============================================
##     RUN
##=============================================
...
##   SAVEDIR
##=============================================
next_dir("membrane2D_squareHoles_lowerFillRatio")
space.make_movie(steps = 20000,filename = "run.gif")
inp = space.get_a_flux("input")
out = space.get_a_flux("output")

space.my_structure.ghost = True
space.make_movie(steps = 20000,filename = "cal.gif")

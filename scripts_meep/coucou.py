...
membrane = membrane2D(height = 0.627, width = 0.62, n = 3.214, centerX = gridSizeX/2., centerY = gridSizeY/2.,centerZ = gridSizeZ/2.)
space.add_structure(membrane)

space.add_flat_source(vol = volume(vec(0,0,1.0),vec(gridSizeX,gridSizeY,1.0)),comp = Ex)
...
##   SAVEDIR
##=============================================
next_dir("membrane2D_justRefl_simily1D")
save_this_script()
space.make_movie(steps = 1000,filename = "run.gif")
inp = space.get_a_flux("input")
out = space.get_a_flux("output")

space.my_structure.ghost = True
space.make_movie(steps = 1000,filename = "cal.gif")
